...
import {
  ...
  updateRowInSheet,
  deleteRowFromSheet,
} from "../src/services/googleSheetService.js";
...
function mapGuestToSheetRow(guest: any) {
  return {
    ...
    attending:
      guest.attending !== undefined ? String(guest.attending) : "",
  ...
  if (request.method === "POST") {
    ...
    try {
      ...
      if (guestIndex === -1) {
        return response
          .status(404)
          .json({ message: `Guest with ID ${id} not found.` });
      }
      ...
      await updateRowInSheet(
        "Guests",
        guestIndex,
        mapGuestToSheetRow(updatedGuest)
      );
      response
        .status(200)
